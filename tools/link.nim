...
proc makeBin(ihxFileName: string): seq[byte] =
  ...
  while s.readLine(o):
    ...
    if (
      let someHighAddr =
        uint16(int(i.records[^1].address) + i.records[^1].data.len())
...
when isMainModule:
  ...
  when true:
    (outfDir / outfName & ".ihx").ihx2bin(
      outfDir / outfName & outfExt, romTitle
    )
